    ...
    <div className="font-outfit h-full flex flex-col max-w-5xl mx-auto">
      <header className="flex justify-between items-center mb-8">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <p className="text-neutral-400 mt-2">AI-detected anomalies requiring immediate triage.</p>
        </motion.div>
        
        <div className="flex bg-black/40 border border-white/10 rounded-xl p-1 backdrop-blur-md">
          <button onClick={() => setFilter('active')} className={`px-4 py-2 rounded-lg text-sm font-medium transition flex items-center gap-2 ${filter === 'active' ? 'bg-red-500/20 text-red-400 shadow-lg' : 'text-neutral-500 hover:text-white'}`}>
            <span className="w-2 h-2 rounded-full bg-red-500"></span> Action Required
          </button>
          <button onClick={() => setFilter('acknowledged')} className={`px-4 py-2 rounded-lg text-sm font-medium transition flex items-center gap-2 ${filter === 'acknowledged' ? 'bg-white/10 text-white shadow-lg' : 'text-neutral-500 hover:text-white'}`}>
            <span className="w-2 h-2 rounded-full bg-emerald-500"></span> Resolved
          </button>
          <button onClick={() => setFilter('all')} className={`px-4 py-2 rounded-lg text-sm font-medium transition ${filter === 'all' ? 'bg-white/10 text-white shadow-lg' : 'text-neutral-500 hover:text-white'}`}>All</button>
        </div>
      </header>
        ...
        <AnimatePresence>
          {filteredAlerts.length === 0 ? (
            ...
            >
              ...
              <p className="text-neutral-400 text-center">All patients are stable. Excellent work.</p>
            </motion.div>
          ) : (
              ...
              >
                <div className="flex gap-4">
                  <div className="mt-1">
                    ...
                    {alert.type === 'system' && <div className="w-10 h-10 bg-neutral-500/20 text-neutral-400 rounded-full flex items-center justify-center"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M18.364 5.636a9 9 0 010 12.728m0 0l-2.829-2.829m2.829 2.829L21 21M15.536 8.464a5 5 0 010 7.072m0 0l-2.829-2.829m-4.243 2.829a4.978 4.978 0 01-1.414-2.83m-1.414 5.658a9 9 0 01-2.167-9.238m7.824 2.167a1 1 0 111.414 1.414m-1.414-1.414L3 3m8.293 8.293l1.414 1.414"></path></svg></div>}
                  </div>
                  <div>
