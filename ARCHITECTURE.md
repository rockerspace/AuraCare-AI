...
4.  **Vertex AI & Google AI Studio (Gemini 1.5 Pro):** Replaces generic ML pipelines. Handles complex multimodal reasoning (analyzing WebRTC video feeds and spatial audio sentiment) when escalated by the edge agent.
5.  **Real-Time Emergency Escalation:** If severe distress (e.g., a fall) is detected, the A2A Coordinator bypasses standard polling and triggers an instant emergency escalation via Cloud Run, notifying caregivers immediately.
